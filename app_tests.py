...
def test_round_floats_nested(tmp_path):
    params = app.AnalysisParameters(output_folder=str(tmp_path), source_path="")
    pipe = app.AnalysisPipeline(params, app.Queue(), app.threading.Event())

    data = {"a": 1.123456, "b": [0.3333333, {"c": 2.987654}]}
    out = pipe._round_floats(data)
    assert out["a"] == 1.1235
...
def test_load_and_filter_metadata_overall_mode(tmp_path):
    ...
    kept, n_kept, total = app.AppUI._load_and_filter_metadata(
        str(md),
        q_thresh=40,
        f_thresh=0.5,
        mode=app.config.FILTER_MODES["OVERALL"],
        ind_thresh=[0] * len(app.config.QUALITY_METRICS),
        quality_weights=app.config.QUALITY_WEIGHTS,
    )
    assert total == 2 and n_kept == 1
    assert kept[0]["filename"] == "frame_000001.png"


def test_apply_gallery_filters_and_export(tmp_path):
    ...
    ui = app.AppUI()
    preview, status = ui.apply_gallery_filters(
        str(md), str(out_dir), 10, 0.0, app.config.FILTER_MODES["OVERALL"]
    )
    assert len(preview) == 1 and Path(preview[0]).name == "frame_000001.png"
    ...
    msg = ui.export_kept_frames(
        ...
        10,
        0.0,
        app.config.FILTER_MODES["OVERALL"],
        False,  # enable_crop
        "1:1",  # crop_ars
        0,      # crop_padding
    )
    assert "Exported 1/2 frames" in msg
