...
def _create_analysis_context(
    ...
    cuda_available: bool,
    ana_ui_map_keys: list[str],
    ana_input_components: list,
    ...
    if not output_folder_str or isinstance(output_folder_str, bool):
        logger.error(
            f"Output folder is not valid (was '{output_folder_str}', type: {type(output_folder_str)}). This is likely due to a UI argument mapping error.",
            component="analysis",
        )
...
def _wire_recompute_handler(
    ...
    try:
        ...
        masker = _create_analysis_context(
            config, logger, thumbnail_manager, cuda_available, ana_ui_map_keys, ana_input_components, model_registry
        )
