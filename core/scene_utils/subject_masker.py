...
class SubjectMasker:
    ...
    def _initialize_tracker(self) -> bool:
        ...
        try:
            ...
            log_with_component(self.logger, "info", f"Initializing SAM3 tracker: {self.params.tracker_model_name}")
            ...
            if self.dam_tracker is None:
                log_with_component(
                    self.logger,
                    "error",
                    f"Tracker '{self.params.tracker_model_name}' initialization returned None/failed",
                )
                return False
            ...
            log_with_component(
                self.logger,
                "error",
                f"Exception during '{self.params.tracker_model_name}' tracker initialization: {e}",
                exc_info=True,
            )
            return False
    ...
    def run_propagation(
        ...
        try:
            if lowres_video_path.exists():
                log_with_component(self.logger, "info", "Using bounded video-based propagation per scene.")

                # Detect actual frame size from thumbnails to ensure correct SAM3 normalization
                frame_size = (640, 480)  # Fallback
                if self.frame_map and self.thumbnail_manager:
                    ...
                    if sample_thumb is not None:
                        ...
                        log_with_component(
                            self.logger,
                            "info",
                            f"Targeting propagation frame size: {frame_size[0]}x{frame_size[1]}",
                ...
                for scene in scenes_to_process:
                    ...
                    bbox = cast(dict, scene.seed_result).get("bbox")
                    if not bbox:
                        log_with_component(self.logger, "warning", f"No seed for scene {scene.shot_id}, skipping.")
                        continue
                    ...
                    for fn in target_fns:
                        ...
                        res = {
                            ...
                            "seed_type": cast(dict, scene.seed_result).get("details", {}).get("type"),
                            "seed_face_sim": cast(dict, scene.seed_result).get("details", {}).get("seed_face_sim"),
                            "mask_area_pct": area,
                            "mask_empty": is_empty,
                ...
                log_with_component(self.logger, "warning", "video_lowres.mp4 not found, falling back to legacy mode.")
                # Legacy fallback omitted or can be re-implemented if needed.
                pass
        finally:
            self.close()

        log_with_component(self.logger, "success", f"Subject masking complete for {len(mask_metadata)} frames.")
        ...
        try:
            ...
            log_with_component(self.logger, "error", f"Failed to save mask metadata: {e}")
