            ...
            if face_landmarker and any(metrics_to_compute.get(k) for k in ["eyes_open", "yaw", "pitch"]):
                ...
                if landmarker_result.face_blendshapes:
                    ...
                    if metrics_to_compute.get("eyes_open"):
                        self.metrics.eyes_open = 1.0 - max(
                            blendshapes.get("eyeBlinkLeft", 0), blendshapes.get("eyeBlinkRight", 0)
                        )
                        self.metrics.blink_prob = max(
                            blendshapes.get("eyeBlinkLeft", 0), blendshapes.get("eyeBlinkRight", 0)
                        )

                if landmarker_result.facial_transformation_matrixes and any(
                    metrics_to_compute.get(k) for k in ["yaw", "pitch"]
                ):
                    matrix = landmarker_result.facial_transformation_matrixes[0]
                    sy = math.sqrt(matrix[0, 0] * matrix[0, 0] + matrix[1, 0] * matrix[1, 0])
                    singular = sy < 1e-6
                    if not singular:
                        if metrics_to_compute.get("pitch"):
                            self.metrics.pitch = math.degrees(math.atan2(-matrix[2, 0], sy))
                        if metrics_to_compute.get("yaw"):
            ...
            if quality_config.enable_niqe and niqe_metric is not None:
                try:
                    ...
                    if active_mask is not None:
                        active_mask_full = (
                            cv2.resize(mask, (rgb_image.shape[1], rgb_image.shape[0]), interpolation=cv2.INTER_NEAREST)
                            > 128
                        )
                    ...
                    with torch.no_grad(), torch.amp.autocast("cuda", enabled=niqe_metric.device.type == "cuda"):
                        ...
                        niqe_score = max(
                            ...
                            min(
                                100,
                                (main_config.quality_niqe_offset - niqe_raw) * main_config.quality_niqe_scale_factor,
                            ),
                        )
