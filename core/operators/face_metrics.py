...
def _get_face_data(ctx: OperatorContext) -> tuple[Optional[dict], Optional[np.ndarray]]:
    ...
    if "face_blendshapes" in ctx.params or "face_matrix" in ctx.params:
        ...
        return MockMPResult(ctx.params.get("face_blendshapes"), ctx.params.get("face_matrix")), ctx.params.get(
            "face_bbox"
        )
...
class FacePoseOperator:
    ...
    def execute(self, ctx: OperatorContext) -> OperatorResult:
        ...
        try:
            ...
            return OperatorResult(metrics={"yaw": yaw, "pitch": pitch, "roll": roll})
