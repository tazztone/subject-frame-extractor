...
class TestSharedUtils:
    ...
    def test_build_scene_gallery_items(self, mock_imread, tmp_path):
        ...
        # View All, Page 1
        items, index_map, total_pages = build_scene_gallery_items(scenes, "All", str(output_dir), page_num=1, page_size=10)
        assert len(items) == 2
        assert len(index_map) == 2
        assert total_pages == 1

        # View Kept
        items, index_map, total_pages = build_scene_gallery_items(scenes, "Kept", str(output_dir), page_num=1, page_size=10)
        assert len(items) == 1
        assert index_map == [0]

        # Pagination
        items, index_map, total_pages = build_scene_gallery_items(scenes, "All", str(output_dir), page_num=1, page_size=1)
        assert len(items) == 1
