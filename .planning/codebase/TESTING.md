...
- **Wait for App Ready**: Always use `wait_for_app_ready(page)` instead of `networkidle`.
- **Parallel Execution**: UI tests support `pytest-xdist`. Each worker runs its own mock server on a unique port (`8765 + worker_id`).
- **Failure Visibility**: Screenshots are automatically captured on failure and saved to `tests/results/failures/`.
- **Log Isolation**: Mock server logs are saved per-run to `tests/results/logs/mock_app_e2e.log`.
...
- **Status Bar Priority**:
    - **Design Pattern**: The `unified_status` component is the primary target for E2E assertions. Any major backend operation should update the `unified_status` component immediately upon completion/failure to provide a clear signal to Playwright's `expect()` locators.
