...
_nn_mod = _create_mock_module('torch.nn', {'Module': MagicMock, 'functional':...
_mp_mod = _create_mock_module('torch.multiprocessing', {'set_start_method': M...
_torch_mod = _create_mock_module('torch', {'__path__': [], '__version__': '2....
_cv2_mod = MagicMock(name='cv2')
_cv2_mod.__name__ = "<REDACTED_STRING>"
_cv2_mod.__package__ = "<REDACTED_STRING>"
_cv2_mod.__path__ = []
...
_cv2_mod.Laplacian = MagicMock(side_effect=lambda src, ddepth, **kwargs: src....
_cv2_mod.Canny = MagicMock(side_effect=lambda image, *args, **kwargs: np.zero...
...
_cv2_mod.Sobel = MagicMock(side_effect=lambda src, *args, **kwargs: np.zeros_...
...
_cv2_mod.getStructuringElement = MagicMock(side_effect=lambda shape, ksize, *...
_cv2_mod.addWeighted = MagicMock(side_effect=lambda src1, alpha, src2, beta, ...
_cv2_mod.findContours = MagicMock(return_value=([], None))
def _mock_connected_components(*args, **kwargs): ...
_cv2_mod.connectedComponentsWithStats = MagicMock(side_effect=_mock_connected...
_matplotlib_pyplot_mod = MagicMock(name='matplotlib.pyplot')
_matplotlib_ticker_mod = MagicMock(name='matplotlib.ticker')
_matplotlib_mod = _create_mock_module('matplotlib', {'__path__': [], 'pyplot'...
_yt_dlp_utils_mod = _create_mock_module('yt_dlp.utils', {'DownloadError': typ...
_yt_dlp_mod = _create_mock_module('yt_dlp', {'__path__': [], 'utils': _yt_dlp...
_skimage_metrics_mod = _create_mock_module('skimage.metrics', {'structural_si...
_skimage_mod = _create_mock_module('skimage', {'__path__': [], 'metrics': _sk...
_mp_vision_mod = _create_mock_module('mediapipe.tasks.python.vision', {'FaceL...
_mp_tasks_python_mod = _create_mock_module('mediapipe.tasks.python', {'vision...
_mp_tasks_mod = _create_mock_module('mediapipe.tasks', {'python': _mp_tasks_p...
_mp_mod = _create_mock_module('mediapipe', {'__path__': [], 'tasks': _mp_task...
...
def test_cli_main_error_handling(capsys):
    """Test that exceptions raised from cli() are caught in the __main__ block and e..."""
```
...
def test_migrate_database_legacy_v2(tmp_path):
    """Test migration from a legacy v2 database (no schema_versions table, but has e..."""
def test_migrate_database_future_version_mocked(tmp_path):
    """Test migration path when a future version requires migrations but skips past ..."""
...
def test_create_fingerprint_deterministic(tmp_path):
    """Test that fingerprint hashes are deterministic regardless of dict insertion o..."""
...
class TestSceneDetection:
    ...
    @patch('cv2.resize')
    @patch('cv2.cvtColor')
    def test_make_photo_thumbs_caching(self, mock_cvtcolor, mock_resize, mock_imread, mock_logger, mock_config_simple, tmp_path):
