...
|------|----------|-----------|--------|
| 2026-03-20 | **No Dirty Submodules (SAM3 Patching)** | **NEVER** modify source code in `SAM3_repo` or other third-party submodules. Dirtying submodules breaks updates and isolation. | All 3rd-party fixes (e.g. `pkg_resources` deprecation, HWC image handling, BFloat16 stability) MUST be implemented via monkey-patching in `core/sam3_patches.py`. |
